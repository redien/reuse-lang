...
type ('Tcollection,'Telement) indexed_45iterator_45class = CIndexedIteratorClass : ('Tcollection -> (('Telement) maybe,'Tcollection) pair) * ('Tcollection -> int32) -> ('Tcollection,'Telement) indexed_45iterator_45class;;
let rec indexed_45iterator_45next = fun _class collection -> (match _class with (CIndexedIteratorClass (next,_95)) -> (next collection));;
let rec indexed_45iterator_45index = fun _class collection -> (match _class with (CIndexedIteratorClass (_95,index)) -> (index collection));;
let rec indexed_45iterator_45from = fun get -> (CIndexedIteratorClass ((fun iterator -> (match iterator with (CPair (collection,i)) -> (CPair ((get collection i),(CPair (collection,(Int32.add i (1l)))))))),(fun iterator -> (match iterator with (CPair (_95,i)) -> i))));;
...
let rec list_45collect_45from_45indexed_45iterator_39 = fun i predicate iterator initial -> (match (indexed_45iterator_45next i iterator) with (CPair (CNone,_95)) -> (CPair (iterator,initial)) | (CPair ((CSome (x)),next)) -> (match (predicate x) with CTrue -> (list_45collect_45from_45indexed_45iterator_39 i predicate next (CCons (x,initial))) | CFalse -> (CPair (iterator,initial))));;
let rec list_45collect_45from_45indexed_45iterator = fun i predicate iterator -> (match (list_45collect_45from_45indexed_45iterator_39 i predicate iterator CEmpty) with (CPair (iterator,result)) -> (CPair (iterator,(list_45reverse result))));;
...
let rec string_45collect_45from_45indexed_45iterator = fun i predicate iterator -> (pair_45map_45right string_45from_45list (list_45collect_45from_45indexed_45iterator i predicate iterator));;
let rec string_45from_45indexed_45iterator = fun i iterator -> (string_45from_45list (pair_45right (list_45collect_45from_45indexed_45iterator i (fun _95 -> CTrue) iterator)));;
