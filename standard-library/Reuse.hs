...
data Tindexed_45iterator_45class collection element = CIndexedIteratorClass !(collection -> (Tpair (Tmaybe element) collection)) !(collection -> Int32)
indexed_45iterator_45next = \ !_class !collection -> (case _class of (CIndexedIteratorClass !next !_95) -> (next collection))
indexed_45iterator_45index = \ !_class !collection -> (case _class of (CIndexedIteratorClass !_95 !index) -> (index collection))
indexed_45iterator_45from = \ !get -> (CIndexedIteratorClass (\ !iterator -> (case iterator of (CPair !collection !i) -> (CPair (get collection i) (CPair collection (_int32_add i 1))))) (\ !iterator -> (case iterator of (CPair !_95 !i) -> i)))
...
list_45collect_45from_45indexed_45iterator_39 = \ !i !predicate !iterator !initial -> (case (indexed_45iterator_45next i iterator) of (CPair CNone !_95) -> (CPair iterator initial) ; (CPair (CSome !x) !next) -> (case (predicate x) of CTrue -> (list_45collect_45from_45indexed_45iterator_39 i predicate next (CCons x initial)) ; CFalse -> (CPair iterator initial)))
list_45collect_45from_45indexed_45iterator = \ !i !predicate !iterator -> (case (list_45collect_45from_45indexed_45iterator_39 i predicate iterator CEmpty) of (CPair !iterator !result) -> (CPair iterator (list_45reverse result)))
...
string_45collect_45from_45indexed_45iterator = \ !i !predicate !iterator -> (pair_45map_45right string_45from_45list (list_45collect_45from_45indexed_45iterator i predicate iterator))
string_45from_45indexed_45iterator = \ !i !iterator -> (string_45from_45list (pair_45right (list_45collect_45from_45indexed_45iterator i (\ !_95 -> CTrue) iterator)))
