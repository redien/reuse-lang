...
run() {
    case $1 in
        ...
        test-interpreter)               $project_root/interpreter/build.sh && IMPL=interpreter run test-minimal && IMPL=interpreter run test-extended ;;
        test-compilers)                 run test-ocaml-compiler && run test-haskell-compiler && run test-javascript-compiler ;;
        test-module-compiler)           $project_root/interpreter/build.sh && $project_root/cli/build.sh && IMPL=compiler-backend/module run test-minimal && IMPL=compiler-backend/module run test-extended ;;
        test-ocaml-compiler)            $project_root/cli/build.sh && IMPL=compiler-backend/ocaml SOURCE=executable.ml run test-minimal && IMPL=compiler-backend/ocaml SOURCE=executable.ml run test-extended ;;
        test-haskell-compiler)          $project_root/cli/build.sh && IMPL=compiler-backend/haskell SOURCE=Executable.hs run test-minimal && IMPL=compiler-backend/haskell SOURCE=Executable.hs run test-extended ;;
        test-javascript-compiler)       $project_root/cli/build.sh && IMPL=compiler-backend/javascript SOURCE=executable.js run test-minimal && IMPL=compiler-backend/javascript SOURCE=executable.js run test-extended ;;
