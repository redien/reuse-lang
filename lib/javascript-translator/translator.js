...
var translateFunctionApplication = function (functionExpression, imports) {
    if (imports.indexOf('stdlib/vector.ru') !== -1) {
        ...
        if (functionName === 'vector:new') {
            ...
            return '(function (vector) { return vector.push(' + translateFunctionArguments(functionExpression, 2, 1, imports) + '), vector; })(' + translateExpression(ast.expression(functionExpression, 1), imports) + ')'
    ...
    try {
        ...
        for (statementIndex = 0; statementIndex < statementCount; ++statementIndex) {
            ...
            if (statementKind === 'export') {
                ...
                // modules.
                imports.push(ast.value(ast.expression(statement, 1)));
