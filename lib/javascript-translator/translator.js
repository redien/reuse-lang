...
var translateLambda = function (ast) {
    return '(function ' + (expressionContainsRecur(ast.elements[2]) ? 'recur' : '') + '(' + translateParameters(ast.elements[1].elements, 0, 1) + ') { ' +
...
exports.translate = function (ast) {
    ...
    try {
        ...
        for (exportIndex = 0; exportIndex < ast.elements.length; ++exportIndex) {
            ...
            var exportAst = ast.elements[exportIndex];
            var name = exportAst.elements[1].value;

            translatedString +=
                'var ' + name + ' = ' + translateExpression(exportAst.elements[2]) +
