    ...
    (let (encode-with-accumulator (lambda (name index encoded-name)
        ...
        (if (< index name-length)
            (if (or (and (>= code-point 65) (<= code-point 90))
                ...
                (recur name (+ index 1) (string:push encoded-name code-point))
                (recur name (+ index 1) (string:concatenate (string:push encoded-name 95) (string:decimal-string-from-integer code-point))))
            encoded-name)))))
