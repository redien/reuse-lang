...
var lookupVariableInScope = function (variable, scope) {
    if (scope[variable]) {
        return {error: null, value: scope[variable]};
    } else if (scope._parentScope) {
        return lookupVariableInScope(variable, scope._parentScope);
    } else {
        return {error: new Error('Found unbound variable')};
    }
};

var variableExistsInParameters = function (variable, parameters) {
    var exists = false;
...
var expressionHasUnboundVariable = function (expression, parameters, scope) {
    if (expression.kind === 'atom' &&
        !isInteger(expression.value)) {

        var lookup = lookupVariableInScope(expression.value, scope);
        if (!variableExistsInParameters(expression.value, parameters) && lookup.error) {
            return true;
        }
    } else if (expression.kind === 'list' && expression.elements.length > 0) {
        if (expression.elements[0].kind === 'atom' &&
            expression.elements[0].value === 'lambda') {
            var newScope = {_parentScope: scope};
            parameters.forEach(function (parameter) {
                newScope[parameter.value] = true;
            });
            return expressionHasUnboundVariable(
                expression.elements[2],
                expression.elements[1].elements,
                newScope
            );

        } else {
            var hasUnbound = false;
            expression.elements.forEach(function (expression) {
                if (expressionHasUnboundVariable(expression, parameters, scope)) {
                    hasUnbound = true;
                }
            });
            return hasUnbound;
...
var evaluate = function evaluate(parsedProgram, scope) {
    if (parsedProgram.kind === 'list') {
        if (parsedProgram.elements.length === 0) {
            ...
            if (first.kind === 'atom' && first.value === 'lambda') {
            ...
            } else if (first.kind === 'atom' && first.value === 'tuple') {
                // (tuple x y)
                ...
                if (arguments.length > f.parameters.length) {
                    return {error: new Error('Unexpected argument in function call')};
                } else if (arguments.length < f.parameters.length) {
                    return {error: new Error('Too few arguments in function call')};
                }
        ...
        if (isInteger(parsedProgram.value)) {
            ...
            return evaluate(lookup.value, scope);
        }
    }
};
